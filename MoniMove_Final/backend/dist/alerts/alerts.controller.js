...
let AlertsController = class AlertsController {
    ...
    inferSeverity(alertType) {
        ...
        if (lower.includes('ngã') || lower.includes('va chạm') || lower.includes('tilt') || lower.includes('crash'))
            return 'critical';
        if (lower.includes('cảnh báo') || lower.includes('pin') || lower.includes('tốc độ'))
            return 'warning';
        return 'info';
    }
};
...
__decorate([
    (0, common_1.Post)(),
    (0, swagger_1.ApiOperation)({ summary: 'Tạo cảnh báo mới', description: 'Lưu alert vào Firebase RTDB, push realtime qua WebSocket và gửi email SOS nếu khẩn cấp' }),
    ...
    (0, swagger_1.ApiResponse)({ status: 403, description: 'Không có quyền truy cập thiết bị này' }),
...
__decorate([
    (0, common_1.Get)(),
    (0, swagger_1.ApiOperation)({ summary: 'Lấy danh sách cảnh báo', description: 'Admin thấy tất cả, User chỉ thấy thiết bị được cấp quyền' }),
    (0, swagger_1.ApiQuery)({ name: 'deviceId', required: false, description: 'Lọc theo thiết bị cụ thể' }),
...
__decorate([
    ...
    (0, swagger_1.ApiOperation)({ summary: 'Test SMTP email', description: 'Gửi email kiểm tra — không cần auth. Body: { toEmail?: string }' }),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
