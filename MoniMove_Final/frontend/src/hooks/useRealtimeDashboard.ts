...
import { subscribeDevicePosition, subscribeDeviceInfo } from '../services/firebaseRealtime';
import { classifyMotion, type MotionClassification } from '../services/motionClassifier';
import { useWebSocket, type WsDeviceUpdate, type WsAlertEvent } from './useWebSocket';
...
export function useRealtimeDashboard(deviceId: string | null) {
    ...
    const [motionState, setMotionState] = useState<MotionClassification | null>(null);
    ...
    const handleDeviceUpdate = useCallback((update: WsDeviceUpdate) => {
        ...
        setMetrics((prev) => ({
            ...
            connectionQuality: rate > 10 ? 'excellent' : rate > 4 ? 'good' : rate > 1 ? 'poor' : 'offline',
        }));
    }, [deviceId]);

    const handleAlert = useCallback((alert: WsAlertEvent) => {
        setAlerts((prev) => [alert, ...prev].slice(0, 10));
        setMetrics((prev) => ({ ...prev, lastAlerts: [alert, ...prev.lastAlerts].slice(0, 10) }));
    }, []);
    ...
    useEffect(() => {
        ...
        const unsubInfo = subscribeDeviceInfo(deviceId, (info: any) => {
            ...
            setLiveData((prev) => {
                ...
                return {
                    ...prev,
                    battery: info.battery != null ? info.battery
                        : info.current_data?.battery != null ? info.current_data.battery
                            : prev.battery,
        ...
        const unsubPos = subscribeDevicePosition(deviceId, (pos) => {
            setLiveData((prev) => {
                ...
                return { ...prev, lat: pos.lat, lng: pos.lng, speed: pos.speed ?? prev.speed };
            });
            setLastUpdateAt(Date.now());
        });
