...
/**
 * useWebSocket — kết nối tới MoniMove Realtime WebSocket Gateway
 *
 * Events nhận được:
...
export function useWebSocket(options: UseWebSocketOptions = {}) {
    ...
    useEffect(() => { onDeviceUpdateRef.current = onDeviceUpdate; }, [onDeviceUpdate]);
    useEffect(() => { onAlertRef.current = onAlert; }, [onAlert]);
    useEffect(() => { onDeviceStatusRef.current = onDeviceStatus; }, [onDeviceStatus]);
