...
function getSensitivityGForce(): number {
  ...
  const map: Record<number, number> = { 1: 4.0, 2: 3.2, 3: 2.5, 4: 2.0, 5: 1.5 };
...
const ALERT_RULES: AlertRule[] = [
  // 1. Ngã đổ (Tilt detection)
  {
    ...
    condition: (data) => {
      ...
      if (total > 0 && Math.acos(Math.min(az / total, 1)) * (180 / Math.PI) > threshold) return true;
      return Math.abs(mpu.angle_x ?? 0) > threshold || Math.abs(mpu.angle_y ?? 0) > threshold;
    },
    message: (data) => {
      const mpu = data.mpu6050;
      if (mpu?.is_tilted) return 'Cảnh báo nghiêm trọng: Cảm biến phát hiện thiết bị bị ngã! Cần hỗ trợ khẩn cấp.';
      const ax = Math.abs((mpu?.accel?.x ?? mpu?.accel_x ?? 0) / G);
      const ay = Math.abs((mpu?.accel?.y ?? mpu?.accel_y ?? 0) / G);
      const az = Math.abs((mpu?.accel?.z ?? mpu?.accel_z ?? G) / G);
      const total = Math.sqrt(ax * ax + ay * ay + az * az);
      const tiltAngle = total > 0 ? Math.acos(Math.min(az / total, 1)) * (180 / Math.PI) : 0;
...
async function createAlert(alert: {
  ...
  try {
    ...
    console.log(`✅ Alert created via Backend API: ${alert.type} for ${alert.deviceId}`);
  } catch (error) {
