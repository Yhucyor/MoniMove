...
import { buildReport, exportReportCsv, exportReportJson, type ReportRange } from '../../services/reportExport';
...
export default function AnalyticsTab() {
    ...
    const loadReport = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const rangeMs: Record<ReportRange, number> = { day: 86400000, week: 604800000, month: 2592000000 };
            const now = Date.now();
            ...
            const positions = useKalman
                ? filterGpsTrack(rawPositions)
                : rawPositions;
    ...
    // Build chart data
    const speedData = report?.positions
        .filter((_, i) => i % Math.max(1, Math.floor((report?.positions.length ?? 1) / 50)) === 0)
        .map((p, i) => ({
            t: new Date(p.timestamp).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' }),
            speed: parseFloat((p.speed ?? 0).toFixed(1)),
            idx: i,
        })) ?? [];

    const alertPieData = Object.entries(report?.alertsByType ?? {}).map(([name, value]) => ({ name, value }));

    // Group by day for bar chart
    const dayMap: Record<string, { distance: number; count: number }> = {};
    report?.positions.forEach((p) => {
        const day = new Date(p.timestamp).toLocaleDateString('vi-VN', { weekday: 'short', month: 'short', day: 'numeric' });
        if (!dayMap[day]) dayMap[day] = { distance: 0, count: 0 };
        dayMap[day].count++;
    });
    const dailyData = Object.entries(dayMap).slice(-14).map(([day, d]) => ({ day, points: d.count }));
        ...
        <div className="space-y-6 animate-in fade-in duration-300">
            ...
            {loading ? (
                ...
                <>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        {[
                            { label: 'Tổng điểm GPS', value: report.totalPoints.toLocaleString(), color: 'text-slate-900', sub: report.totalPoints === 0 ? 'Chưa có data' : `~${(report.totalPoints / Math.max(1, (report.to - report.from) / 3600000)).toFixed(0)} pts/h` },
                            { label: 'Quãng đường', value: `${report.totalDistanceKm.toFixed(2)} km`, color: 'text-[#00b494]', sub: report.totalDistanceKm === 0 ? 'Không di chuyển' : 'Haversine' },
                            { label: 'Tốc độ tối đa', value: report.maxSpeedKmh > 0 ? `${report.maxSpeedKmh.toFixed(0)} km/h` : '—', color: report.maxSpeedKmh > 80 ? 'text-red-500' : 'text-amber-600', sub: report.avgSpeedKmh > 0 ? `TB: ${report.avgSpeedKmh.toFixed(1)} km/h` : '' },
                            { label: 'Tổng sự cố', value: report.alertCount, color: report.alertCount > 0 ? 'text-red-500' : 'text-emerald-600', sub: report.alertCount > 0 ? `${Object.keys(report.alertsByType).length} loại` : 'An toàn' },
                        ].map((s) => (
                    ...
                    <div className="flex items-center gap-2 rounded-xl border border-slate-100 bg-slate-50 px-4 py-2.5 text-[10px] text-slate-500 font-medium">
                        ...
                        <span>{new Date(report.from).toLocaleString('vi-VN')} → {new Date(report.to).toLocaleString('vi-VN')}</span>
                        <span className="ml-auto font-bold text-slate-600">{report.deviceId}</span>
                    </div>

                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
                            <p className="text-[9px] font-bold uppercase tracking-wider text-slate-400">Tốc độ TB</p>
                            <p className="mt-1 text-xl font-black text-blue-600">{report.avgSpeedKmh.toFixed(1)} km/h</p>
                        </div>
                        <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
                            <p className="text-[9px] font-bold uppercase tracking-wider text-slate-400">TG di chuyển</p>
                            <p className="mt-1 text-xl font-black text-cyan-600">{Math.round(report.activeTimeMinutes)} phút</p>
                        </div>
                        <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
                            <p className="text-[9px] font-bold uppercase tracking-wider text-slate-400">TG đứng yên</p>
                            <p className="mt-1 text-xl font-black text-slate-500">{Math.round(report.stationaryTimeMinutes)} phút</p>
                        </div>
                        <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
                            <p className="text-[9px] font-bold uppercase tracking-wider text-slate-400">Lọc GPS</p>
                            <p className={`mt-1 text-xs font-black ${useKalman ? 'text-[#00b494]' : 'text-slate-400'}`}>
                        ...
                        <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
                            ...
                            <ResponsiveContainer width="100%" height={200}>
                                <LineChart data={speedData}>
                                    ...
                                    />
                                    <Line type="monotone" dataKey="speed" stroke="#00b494" strokeWidth={2} dot={false} />
                                </LineChart>
                            </ResponsiveContainer>
                        </div>
                    )}
                    ...
                    {dailyData.length > 0 && (
                        <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
                            <h3 className="text-sm font-bold text-slate-800 mb-4">Hoạt động theo ngày (số điểm GPS)</h3>
                    ...
                    {alertPieData.length > 0 && (
                        <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
                            <h3 className="text-sm font-bold text-slate-800 mb-4">Phân loại sự cố</h3>
                            <ResponsiveContainer width="100%" height={200}>
                                <PieChart>
                                    <Pie data={alertPieData} cx="50%" cy="50%" outerRadius={70} dataKey="value" label={({ name, percent }) => `${name} ${((percent || 0) * 100).toFixed(0)}%`} labelLine={false}>
                ...
                <div className="flex flex-col items-center justify-center py-12 border border-dashed border-slate-200 rounded-2xl text-center">
                    ...
                    <p className="text-sm font-bold text-slate-600 mb-1">Chưa có dữ liệu GPS</p>
