...
export default function AlertsHistoryTab({ showAllDevices = false }: AlertsHistoryTabProps) {
  ...
  const fetchLogs = async (showRefreshIndicator = false) => {
    ...
    try {
      // Backend tự lọc theo token — admin thấy tất cả, user thấy thiết bị của mình
      const data = await getAlertsHistory();
      // Nếu là user (không phải admin) và không có showAllDevices, chỉ lấy alerts của thiết bị mình
      setLogs(data);
      setError(null);
    } catch (err: any) {
      ...
      {loading ? (
        ...
        <div className="relative border-l border-slate-200/60 ml-4.5 pl-6.5 space-y-5">
          {logs.map((log) => {
            ...
            const isTilt = alertType.includes('ngã') || alertType.includes('Ngã') || alertType.includes('Tilt');
            ...
            return (
              ...
              >
                ...
                <div className={`absolute -left-[32px] top-6 h-3.5 w-3.5 rounded-full border-[3px] border-white shadow-sm transition-all duration-300 group-hover:scale-110 ${isTilt ? 'bg-red-500 shadow-red-200' : 'bg-amber-500 shadow-amber-200'
                  }`} />

                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                  <div className="flex items-start gap-3.5">
                    {/* Icon container */}
                    <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border ${isTilt
                    ...
                    <div className="space-y-1">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className={`inline-block rounded-lg px-2 py-0.5 text-[9px] font-extrabold uppercase tracking-wide border ${isTilt
