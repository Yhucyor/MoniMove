...
function calcTiltAngles(accel: { x: number; y: number; z: number } | undefined): { pitch: number; roll: number } {
...
function calcTotalAccel(accel: { x: number; y: number; z: number } | undefined): number {
  if (!accel) return 0;
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex justify-between items-center text-[10px] font-bold">
        ...
        <span className={textClass}>{value.toFixed(1)}{unit}</span>
      </div>
...
export default function DeviceCard({ device, viewMode = 'grid' }: DeviceCardProps) {
  ...
  return (
    <div className={`group relative overflow-hidden rounded-[32px] border border-slate-200/50 bg-white/90 backdrop-blur-xl p-6 shadow-lg hover:shadow-xl hover:border-cyan-300/40 transition-all duration-500 ease-out transform hover:-translate-y-1 ${isList ? 'w-full flex flex-col md:flex-row gap-6' : 'w-full flex flex-col'}`}>
      ...
      {/* HEADER */}
      <div className={`mb-4 flex items-start justify-between border-slate-100 pb-4 ${isList ? 'md:w-1/4 md:border-r md:border-b-0 md:pr-4 md:mb-0' : 'border-b'}`}>
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-mono">{device.id}</span>
          ...
          <h3 className="text-lg font-extrabold text-slate-900 group-hover:text-cyan-600 transition-colors">{device.name}</h3>
        </div>
        ...
        <div className="rounded-[24px] border border-cyan-100/50 bg-gradient-to-br from-cyan-50/50 via-white to-transparent p-5 relative overflow-hidden group/gps">
          ...
          {gps ? (
            <div className="space-y-3">
              <div className="flex items-center gap-3 bg-white/80 border border-slate-100 rounded-xl px-4 py-3 relative shadow-sm">
                ...
                <div className="pl-6 grid grid-cols-2 gap-4 w-full">
                  <div>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-0.5">Vĩ độ</p>
                    <p className="font-bold text-slate-800 font-mono text-sm">{gps.latitude.toFixed(6)}°</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-0.5">Kinh độ</p>
                    <p className="font-bold text-slate-800 font-mono text-sm">{gps.longitude.toFixed(6)}°</p>
                  </div>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className={`rounded-xl border px-4 py-3 flex items-center gap-3 shadow-sm ${speedKmh >= 80 ? 'bg-red-50 border-red-100' : 'bg-white border-slate-100'}`}>
                  <div className={`p-2 rounded-full ${speedKmh >= 80 ? 'bg-red-100' : 'bg-blue-50'}`}>
                    <Navigation className={`h-4 w-4 ${speedKmh >= 80 ? 'text-red-600' : 'text-blue-600'}`} />
                  </div>
                  <div>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Tốc độ</p>
                    <p className={`font-black text-base ${speedKmh >= 80 ? 'text-red-600' : 'text-blue-700'}`}>{speedKmh.toFixed(0)} <span className="text-[10px] font-semibold">km/h</span></p>
                  </div>
                </div>
                <div className="rounded-xl bg-white border border-slate-100 px-4 py-3 flex items-center gap-3 shadow-sm">
                  ...
                  <div>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Vệ tinh</p>
                    <p className="font-black text-base text-indigo-700">{gps.satellites ?? '—'} <span className="text-[10px] font-semibold">sats</span></p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-6 text-slate-400 bg-white/50 rounded-xl border border-dashed border-slate-200">
              ...
              <p className="text-xs font-semibold">Chưa nhận được tín hiệu GPS</p>
            </div>
          )}
        </div>
          ...
          {mpu ? (
            <div className="space-y-4 relative z-10">
              <div className="bg-white/80 border border-slate-100 rounded-xl p-4 shadow-sm space-y-3">
                {renderProgressBar('Pitch', pitch, 90)}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className={`rounded-xl border bg-white px-4 py-3 shadow-sm ${dynamicG > 1.5 ? 'border-red-200' : 'border-slate-100'}`}>
                  ...
                  <p className={`font-black text-base ${dynamicG > 1.5 ? 'text-red-600' : 'text-slate-700'}`}>
                    {totalAccelG.toFixed(2)}<span className="text-[10px] font-semibold ml-0.5">G</span>
                  </p>
                </div>
                <div className={`rounded-xl border bg-white px-4 py-3 shadow-sm ${isTilted ? 'border-red-200' : 'border-slate-100'}`}>
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider flex items-center gap-1 mb-1"><ShieldAlert className="h-3 w-3" /> Fall Status</p>
                  <p className={`font-black text-base ${isTilted ? 'text-red-600' : 'text-emerald-600'}`}>
            ...
            <div className="flex flex-col items-center justify-center py-6 text-slate-400 bg-white/50 rounded-xl border border-dashed border-slate-200">
              ...
              <p className="text-xs font-semibold">Chưa nhận được tín hiệu IMU</p>
            </div>
          )}
        </div>

        {/* Footer */}
        <div className={`flex items-center justify-between text-[10px] text-slate-400 font-medium pt-2 ${isList ? 'xl:col-span-2' : ''}`}>
          ...
          <span className="font-mono bg-slate-50 px-2 py-1 rounded-md border border-slate-100">
            {device.lastUpdate ? new Date(device.lastUpdate).toLocaleTimeString('vi-VN') : '—'}
          </span>
        </div>
      </div>
    </div>
