...
<body style="margin:0;padding:0;background:#f1f5f9;font-family:'Segoe UI',Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="padding:32px 16px;">
    ...
    try {
      ...
      this.logger.log(
        `✅ Emergency email sent to ${toEmail} | messageId: ${info.messageId}`,
      );
    } catch (error) {
      this.logger.error(
        `❌ Failed to send email to ${toEmail}: ${error instanceof Error ? error.message : String(error)}`,
      );
