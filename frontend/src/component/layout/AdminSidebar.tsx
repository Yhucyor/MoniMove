...
export default function AdminSidebar({ isOpen, onClose, activeTab, onTabChange }: AdminSidebarProps) {
  ...
  return (
    <aside
      ...
      className={`fixed left-0 top-0 z-50 flex h-full w-48 flex-col border-r border-slate-200/50 bg-gradient-to-tr from-[#eef7f8] via-[#f4f3f8] to-[#fbeff5] pl-3 py-3 pr-0 shadow-2xl transition-transform duration-300 ease-in-out overflow-hidden animated-gradient-bg ${isOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
    >
      ...
      <div className="relative z-10 flex flex-col gap-6">
        {/* Logo */}
        <div className={`flex select-none items-center gap-2 px-1.5 py-1 pr-4 transition-all duration-500 ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2 pointer-events-none'}`}>
          ...
          <div>
            ...
            <p className="text-[8px] font-bold uppercase tracking-widest text-[#00b494]/80">Admin Panel</p>
          </div>
        </div>
          ...
          {menuItems.map((item, index) => {
            ...
            return (
              <button
                ...
                className={`group flex w-full items-center gap-2.5 py-2.5 text-xs font-semibold transition-all duration-500 transform relative ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'
                  } ${isActive
