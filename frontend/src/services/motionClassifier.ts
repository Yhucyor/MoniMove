...
function extractFeatures(s: SensorSnapshot) {
    ...
    return { totalAccel, totalGyro, tiltAngle, dynamicAccel, ax, ay, az, gx, gy, gz };
}
...
export function classifyMotion(snapshot: SensorSnapshot): MotionClassification {
    ...
    if (snapshot.is_tilted === true || f.tiltAngle > 60) {
        const conf = snapshot.is_tilted ? 0.95 : Math.min(0.5 + (f.tiltAngle - 60) / 60, 0.9);
        reasons.push(`Góc nghiêng ${f.tiltAngle.toFixed(0)}°`);
        if (snapshot.is_tilted) reasons.push('Cảm biến is_tilted=true');
        return { state: 'fallen', confidence: conf, label: 'Ngã đổ', severity: 'critical', reasons };
    }
    ...
    if (f.totalAccel > 2.5 || f.dynamicAccel > 1.5) {
        ...
        if (f.totalGyro > 3) reasons.push(`Con quay ${f.totalGyro.toFixed(1)} rad/s`);
        return { state: 'impact', confidence: conf, label: 'Va chạm mạnh', severity: 'critical', reasons };
    }

    // ── FAST MOVING ─────────────────────────────────────────────────────────
    if (speed > 40) {
        reasons.push(`Tốc độ ${speed.toFixed(0)} km/h`);
        return { state: 'fast_moving', confidence: 0.9, label: 'Di chuyển nhanh', severity: speed >= 80 ? 'warning' : 'normal', reasons };
    }
    ...
    if (speed > 5 || f.dynamicAccel > 0.15 || f.totalGyro > 0.5) {
        ...
        reasons.push(`Tốc độ ${speed.toFixed(0)} km/h`);
        if (f.dynamicAccel > 0.15) reasons.push(`Gia tốc động ${f.dynamicAccel.toFixed(2)}G`);
        return { state: 'moving', confidence: conf, label: 'Đang di chuyển', severity: 'normal', reasons };
    }

    // ── WALKING ─────────────────────────────────────────────────────────────
    if (speed > 0.5 || f.dynamicAccel > 0.05 || f.totalGyro > 0.1) {
        reasons.push('Rung nhẹ / bước chân');
        return { state: 'walking', confidence: 0.7, label: 'Đi bộ', severity: 'normal', reasons };
    }

    // ── STATIONARY ──────────────────────────────────────────────────────────
    reasons.push('Gia tốc ổn định, tốc độ = 0');
    return { state: 'stationary', confidence: 0.85, label: 'Đứng yên', severity: 'normal', reasons };
}

export function getMotionStateColor(state: MotionState): string {
    switch (state) {
        case 'fallen': return 'text-red-600 bg-red-50 border-red-200';
        case 'impact': return 'text-red-500 bg-red-50 border-red-200';
        case 'fast_moving': return 'text-amber-600 bg-amber-50 border-amber-200';
        case 'moving': return 'text-blue-600 bg-blue-50 border-blue-200';
        case 'walking': return 'text-cyan-600 bg-cyan-50 border-cyan-200';
        case 'stationary': return 'text-emerald-600 bg-emerald-50 border-emerald-200';
