...
export function buildReport(
  ...
  const filtered = positions.filter(
    (p) => p.timestamp >= from && p.timestamp <= now,
  );
  const filteredAlerts = alerts.filter(
    (a) => a.timestamp >= from && a.timestamp <= now,
  );
  ...
  const activeTimeMinutes =
    filtered.length > 0 ? (activePoints / filtered.length) * totalMins : 0;
...
export function exportReportCsv(report: ActivityReport): void {
  const lines: string[] = [];
  const rangeLabel = { day: "Ngày", week: "Tuần", month: "Tháng" }[
    report.range
  ];
  const dateStr = new Date(report.from).toLocaleDateString("vi-VN");

  lines.push(`Báo cáo hoạt động MoveMonitor - ${rangeLabel} ${dateStr}`);
  ...
  lines.push(
    `Thời gian di chuyển (phút),${report.activeTimeMinutes.toFixed(0)}`,
  );
  lines.push(
    `Thời gian đứng yên (phút),${report.stationaryTimeMinutes.toFixed(0)}`,
  );
  ...
  report.positions.forEach((p) => {
    lines.push(
      `${new Date(p.timestamp).toLocaleString("vi-VN")},${p.lat},${p.lng},${(p.speed ?? 0).toFixed(1)}`,
    );
  });
