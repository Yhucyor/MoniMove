...
export async function sendAlert(
  ...
  const payload = { deviceId, alertType, message, severity, location, sosEmail, timestamp: Date.now() };

  if (!isBrowserOnline()) {
    await queuePendingAlert({ deviceId, alertType, message, timestamp: payload.timestamp });
  ...
  try {
    ...
    await queuePendingAlert({ deviceId, alertType, message, timestamp: Date.now() });
    console.warn('Alert queued for sync:', error instanceof Error ? error.message : error);
    return { success: true, queued: true };
  }
}
...
export async function updateUserRole(email: string, role: 'user' | 'admin'): Promise<UserProfile> {
...
export async function updateUserDevices(email: string, deviceIds: string[]): Promise<UserProfile> {
  const response = await fetch(`${API_BASE_URL}/users/${encodeURIComponent(email)}/devices`, {
    method: 'PATCH',
    headers: getAuthHeaders(),
  ...
  try {
    ...
    await cacheDevices(
      data.map((d) => ({
        ...
        name: d.name,
        status: d.status,
        connectionStatus: d.connectionStatus || getConnectionStatus(undefined, d.lastPing ?? undefined, d.status),
        cachedAt: Date.now(),
      })),
    );
    return data;
  } catch (error) {
    console.warn('Error listing devices (using cache):', error instanceof Error ? error.message : error);
...
export async function getAlertsHistory(deviceId?: string): Promise<AlertLog[]> {
  try {
    ...
    await cacheAlerts(
      data.map((a) => ({ ...a, cachedAt: Date.now() })),
    );
    return data;
  } catch (error) {
    console.warn('Error fetching alerts (using cache):', error instanceof Error ? error.message : error);
