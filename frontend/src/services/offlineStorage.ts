...
async function withStore<T>(
  ...
  return new Promise((resolve, reject) => {
    ...
    tx.oncomplete = () =>
      resolve(result instanceof IDBRequest ? undefined : undefined);
...
export async function getCachedPositions(
  deviceId: string,
): Promise<{ lat: number; lng: number; timestamp: number; speed?: number }[]> {
  const db = await openDb();
  return new Promise((resolve, reject) => {
...
export async function queuePendingAlert(
  alert: Omit<PendingAlert, "id" | "createdAt">,
): Promise<string> {
