...
function saveSession(authResponse: any, token: string) {
  ...
  localStorage.setItem(
    "user_device_ids",
    JSON.stringify(authResponse.user.deviceIds || []),
  );
  ...
  function getFirebaseErrorMsg(code: string, fallback: string): string {
    const map: Record<string, string> = {
      ...
      "auth/invalid-continue-uri":
        '⚠️ Cấu hình Firebase chưa đúng. Thêm "localhost" vào Authorized domains.',
      "auth/unauthorized-continue-uri":
        "⚠️ Domain chưa được authorize trong Firebase Console.",
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (!backendHealth.isBackendOnline) {
        throw new Error(
          backendHealth.error ||
            "Backend không phản hồi. Vui lòng khởi động backend trước.",
        );
      }
      ...
      setErrorMsg(
        err.code ? getFirebaseErrorMsg(err.code, err.message) : err.message,
      );
=======
  const handleGoogleLogin = async () => {
...
<<<<<<< HEAD
            ...
            <p className="mt-0.5 text-xs text-slate-400">
              {isRegister
                ? "Đăng ký để bắt đầu giám sát thiết bị"
                : "Đăng nhập vào hệ thống MoveMonitor"}
            </p>
          </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={
                    isRegister ? "email@example.com" : "Nhập email của bạn..."
                  }
                  value={username}
