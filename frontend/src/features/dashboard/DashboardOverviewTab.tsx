    ...
    return (
      <div className="flex flex-col items-center justify-center p-12 border border-dashed border-slate-200 rounded-[24px] bg-white/50 text-center max-w-lg mx-auto mt-8">
        ...
        <p className="mt-2 text-xs text-slate-500">
          Liên hệ Admin để được cấp quyền truy cập thiết bị.
        </p>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          },
          {
            ...
            sub: `${devices.length > 0 ? Math.round((onlineCount / devices.length) * 100) : 0}%`,
            icon: "🟢",
          },
          {
            label: "Tốc độ",
          ...
          <div
            key={s.label}
            className="rounded-2xl border border-slate-100 bg-white px-4 py-3.5 shadow-sm flex items-center gap-3"
          >
            ...
            <div className="min-w-0">
              ...
              <p className={`mt-0.5 text-lg font-black truncate ${s.color}`}>
                {loading ? "—" : s.value}
              </p>
              {s.sub && (
                <p className={`text-[9px] font-semibold ${s.color} opacity-60`}>
                  {s.sub}
                </p>
              )}
            </div>
      ...
      {isWsConnected && (
        <div className="flex items-center gap-3 rounded-xl border border-emerald-100 bg-emerald-50/50 px-4 py-2.5">
          <div className="flex items-end gap-0.5">
            {(["offline", "poor", "good", "excellent"] as const).map(
              (level, i) => {
                const qualityIndex = {
                  offline: 0,
                  poor: 1,
                ...
                return (
                  <div
                    key={level}
                    className={`w-1.5 rounded-sm transition-all ${active ? "bg-emerald-500" : "bg-slate-200"}`}
          ...
          <span className="text-[10px] text-emerald-600">
            {metrics.updatesPerMinute} updates/min
          </span>
          <span className="text-[10px] text-slate-400 ml-auto">
            {metrics.updateCount} tổng điểm dữ liệu
          </span>
        </div>
      )}
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between rounded-2xl border p-4 text-left transition-all duration-200 ${
                      isSelected
                        ? "border-[#00b494]/40 bg-gradient-to-r from-[#00b494]/5 to-[#12a1c0]/5 shadow-sm"
                        : "border-slate-100 bg-white hover:border-[#00b494]/20 hover:shadow-sm"
                    }`}
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <div
                        className={`h-9 w-9 rounded-xl flex items-center justify-center shrink-0 ${isLive ? "bg-emerald-100" : "bg-slate-100"}`}
                      >
                        <span
                          className={`h-2.5 w-2.5 rounded-full ${isLive ? "bg-emerald-500 animate-pulse" : "bg-slate-300"}`}
                        />
                      </div>
                      <div className="min-w-0">
            ...
            <>
              ...
              {liveData && (
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div
                    className={`rounded-xl border p-3.5 text-center ${liveData.isTilted ? "border-red-200 bg-gradient-to-br from-red-50 to-red-50/30 animate-pulse" : "border-slate-100 bg-gradient-to-br from-slate-50 to-slate-50/30"}`}
                  >
                    <Gauge
                      className={`h-4 w-4 mx-auto mb-1.5 ${liveData.isTilted ? "text-red-500" : "text-slate-400"}`}
                    />
                    <p
                      className={`text-sm font-black ${liveData.isTilted ? "text-red-600" : "text-slate-600"}`}
                    >
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <button
                  onClick={() => onNavigate?.("alerts_history")}
