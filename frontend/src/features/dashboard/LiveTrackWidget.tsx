    ...
    <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        ...
        <div className="rounded-xl bg-[#00b494]/5 border border-[#00b494]/20 p-2.5 text-center">
          <p className="text-[9px] font-bold uppercase text-[#00b494]/70 tracking-wider">
            Quãng đường
          </p>
          <p className="text-lg font-black text-[#00b494] mt-0.5">
        ...
        <div className="rounded-xl bg-amber-50 border border-amber-100 p-2.5 text-center">
          ...
          <p
            className={`text-lg font-black mt-0.5 ${stats.maxSpeed >= 80 ? "text-red-500" : "text-amber-600"}`}
          >
      ...
      {stats.points > 0 && (
        <div className="mt-2.5 flex items-center gap-2 text-[10px] text-slate-400 font-medium">
          ...
          Đang ghi hành trình · {Math.floor(stats.durationMin)}p{" "}
          {Math.floor((stats.durationMin % 1) * 60)}s
        </div>
      )}
    </div>
  );
