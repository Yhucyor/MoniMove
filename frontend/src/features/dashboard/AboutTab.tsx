...
import {
  ...
  BarChart2, Wifi, GitBranch, Zap, Lock, Shield,
} from 'lucide-react';

const FEATURES = [
  {
    icon: MapPin, color: 'text-cyan-600', bg: 'bg-cyan-50 group-hover:bg-cyan-100',
    title: 'Giám sát GPS thời gian thực',
    desc: 'Thu thập tọa độ GPS liên tục, hiển thị trực tiếp trên bản đồ số với độ trễ thấp.',
  },
  {
    icon: Activity, color: 'text-red-500', bg: 'bg-red-50 group-hover:bg-red-100',
  ...
  {
    icon: Wifi, color: 'text-indigo-600', bg: 'bg-indigo-50 group-hover:bg-indigo-100',
    title: 'Hỗ trợ offline & đồng bộ',
    desc: 'Lưu dữ liệu tạm thời bằng IndexedDB khi mất mạng, tự động đồng bộ khi kết nối trở lại.',
  },
  {
    icon: Lock, color: 'text-slate-600', bg: 'bg-slate-100 group-hover:bg-slate-200',
...
const TECH_STACK = [
  { label: 'Frontend', icon: '🖥️', color: 'border-blue-100 bg-blue-50/50', items: ['Next.js 15', 'TypeScript', 'Tailwind CSS', 'Recharts'] },
  { label: 'Backend', icon: '⚙️', color: 'border-emerald-100 bg-emerald-50/50', items: ['NestJS', 'Firebase Admin SDK', 'Firestore', 'REST API'] },
  { label: 'Realtime', icon: '⚡', color: 'border-amber-100 bg-amber-50/50', items: ['Firebase Realtime DB', 'WebSocket', 'IndexedDB offline'] },
  { label: 'Hardware', icon: '🔧', color: 'border-purple-100 bg-purple-50/50', items: ['ESP32', 'MPU6050 (IMU)', 'GPS Neo-6M', 'Arduino'] },
];
...
const MEMBERS = [
  {
    ...
    tags: ['ESP32 / Arduino', 'NestJS Backend', 'Next.js 15', 'Firebase', 'WebSocket'],
      ...
      <div className="relative z-10 space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-700 max-w-5xl mx-auto">
            ...
            <div className="space-y-4 flex-1">
              <div className="flex items-center gap-3">
                <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-[#12a1c0] to-[#00b494] shadow-lg shrink-0">
                  <svg viewBox="0 0 24 24" className="h-7 w-7 fill-current text-white" fillRule="evenodd" clipRule="evenodd">
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                {['IoT · GPS Tracking', 'Real-time Alerts', 'AI Motion Detection', 'Offline Support'].map((tag) => (
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {FEATURES.map((f, i) => {
              ...
              return (
                ...
                >
                  <div className={`mb-3 inline-flex h-10 w-10 items-center justify-center rounded-xl transition-all duration-300 ${f.bg}`}>
        ...
        <div className="rounded-2xl border border-slate-200/60 bg-white/90 backdrop-blur-md p-6 shadow-sm">
          ...
          <div className="grid gap-4 md:grid-cols-3 relative">
            ...
            {[
              { icon: Cpu, color: 'text-indigo-500', bg: 'bg-indigo-50 border-indigo-200', num: '01', title: 'Hardware IoT', desc: 'ESP32 + MPU6050 + GPS Neo-6M thu thập tọa độ, gia tốc, con quay hồi chuyển và truyền lên cloud.' },
              { icon: Terminal, color: 'text-emerald-500', bg: 'bg-emerald-50 border-emerald-200', num: '02', title: 'Cloud API & DB', desc: 'NestJS Backend xác thực JWT, phân quyền Role-based. Firebase Realtime DB + Firestore lưu trữ.' },
              { icon: GitBranch, color: 'text-cyan-500', bg: 'bg-cyan-50 border-cyan-200', num: '03', title: 'Web Dashboard', desc: 'Next.js 15 hiển thị bản đồ Leaflet, biểu đồ Recharts, cảnh báo realtime và xuất báo cáo.' },
            ].map((s) => {
              const Icon = s.icon;
              return (
                <div key={s.num} className="group flex flex-col items-center text-center p-4 relative z-10 rounded-2xl border border-slate-100 bg-slate-50/50 hover:bg-white hover:shadow-md transition-all duration-300">
                  <div className={`flex h-14 w-14 items-center justify-center rounded-2xl border-2 mb-3 transition-all duration-300 group-hover:scale-110 ${s.bg}`}>
        ...
        <div className="rounded-2xl border border-slate-200/60 bg-white/90 backdrop-blur-md p-6 shadow-sm">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {TECH_STACK.map((stack) => (
              <div key={stack.label} className={`rounded-xl border p-4 ${stack.color}`}>
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-lg">{stack.icon}</span>
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-600">{stack.label}</p>
                </div>
                <div className="space-y-1.5">
                  {stack.items.map((item) => (
                    <div key={item} className="rounded-lg border border-slate-200/60 bg-white/80 px-2.5 py-1.5 text-[10px] font-semibold text-slate-700">
                      {item}
                    </div>
                  ))}
                </div>
        ...
        <div className="rounded-2xl border border-slate-200/60 bg-white/90 backdrop-blur-md p-6 shadow-sm">
          ...
          {MEMBERS.filter(m => m.isLead).map(member => (
            <div key={member.name} className="mb-4 relative rounded-2xl border border-[#00b494]/25 bg-gradient-to-r from-[#00b494]/8 via-[#0fbbdc]/5 to-transparent p-5 shadow-sm overflow-hidden">
              ...
              <div className="relative z-10 flex flex-col sm:flex-row sm:items-center gap-5">
                ...
                <div className="relative shrink-0">
                  <div className={`absolute inset-0 rounded-full bg-gradient-to-br ${member.gradient} blur-xl opacity-30`} />
                  <div className={`relative flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br ${member.gradient} text-white shadow-lg ring-4 ${member.ring}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    ...
                    <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-[10px] font-bold ${member.badge}`}>
                      Nhóm trưởng
                    </span>
                  </div>
                  <p className="text-xs text-slate-500 leading-relaxed mb-3 max-w-xl">{member.desc}</p>
                  <div className="flex flex-wrap gap-1.5">
                    {member.tags.map(t => (
                      <span key={t} className="rounded-lg bg-white border border-slate-200 px-2 py-0.5 text-[9px] font-bold text-slate-600 uppercase tracking-wide shadow-sm">
                        {t}
                      </span>
                    ))}
                  </div>
                ...
                <div className="shrink-0 hidden sm:block text-right">
                  <p className="text-[9px] font-black uppercase tracking-widest text-[#00b494] mb-1">Vai trò</p>
                  <p className="text-xs font-bold text-slate-700">{member.role.split(' · ').pop()}</p>
                </div>
              </div>
            </div>
          ))}
            ...
            {MEMBERS.filter(m => !m.isLead).map(member => (
              ...
              >
                ...
                <div className="flex items-center gap-3 mb-4">
                  <div className="relative shrink-0">
                    <div className={`absolute inset-0 rounded-full bg-gradient-to-br ${member.gradient} blur-md opacity-20`} />
                    <div className={`relative flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br ${member.gradient} text-white shadow-md ring-2 ${member.ring}`}>
                      <User className="h-5 w-5" />
                    </div>
                  </div>
                  <div className="min-w-0">
                    <p className="text-sm font-extrabold text-slate-900 leading-tight">{member.name}</p>
                    <span className={`inline-block rounded-full border px-2 py-0.5 text-[9px] font-bold mt-1 ${member.badge}`}>
                      {member.role}
                    </span>
                  </div>
                </div>
                ...
                {/* Desc */}
                <p className="text-[11px] text-slate-500 leading-relaxed flex-1 mb-4">{member.desc}</p>

                {/* Tags */}
                <div className="flex flex-wrap gap-1.5 mt-auto">
                  {member.tags.map(t => (
                    <span key={t} className="rounded-md bg-slate-50 border border-slate-100 px-1.5 py-0.5 text-[9px] font-bold text-slate-500 uppercase tracking-wide">
                      {t}
                    </span>
                  ))}
                </div>
