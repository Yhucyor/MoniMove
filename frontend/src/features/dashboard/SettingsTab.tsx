...
import { Sliders, RefreshCw, Compass, Info, Mail, User, Save, Zap, CheckCircle, AlertTriangle } from 'lucide-react';
import { getDeviceSettings, saveDeviceSettings, DeviceSettings } from '../../services/api';
...
export default function SettingsTab() {
  ...
  useEffect(() => {
    ...
    getDeviceSettings(deviceId)
      ...
      .finally(() => { setLoading(false); setHasUnsaved(false); });
  }, [deviceId]);
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      await Promise.all(devices.map(id => saveDeviceSettings(id, { ...settings, sos_email: sosEmailInput })));
      window.dispatchEvent(new CustomEvent('monimove:settings:changed', {
        detail: { sensitivity: settings.sensitivity, tiltThreshold: settings.tilt_threshold },
      }));
      setHasUnsaved(false);
      setToastAll(true);
  ...
  const handleSaveEmail = async () => {
    ...
    try {
      ...
      const devices = user?.deviceIds?.length ? user.deviceIds : [deviceId];
      await Promise.all(devices.map(id =>
        saveDeviceSettings(id, { ...settings, sos_email: sosEmailInput.trim() })
      ));
  ...
  const cardClass = 'rounded-[20px] bg-white p-6 shadow-[0_4px_24px_rgba(0,0,0,0.04)] border border-slate-100/60';
  ...
  return (
    <div className="mx-auto w-full max-w-[1400px] p-6 space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 pb-5 gap-4">
        <div>
          <h2 className="text-[30px] font-bold tracking-tight text-slate-900 leading-tight">Cài đặt thiết bị</h2>
          <p className="mt-1 text-[14px] text-slate-500 font-medium">
            {isAdmin ? 'Chỉnh ngưỡng cảm biến và email SOS.' : 'Cập nhật email SOS khẩn cấp của bạn.'}
          </p>
        </div>
        {isAdmin && (
          <button
            ...
            className={`flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-bold shadow-md transition-all active:scale-95 ${hasUnsaved && !isSavingAll
                ? 'bg-[#00b494] text-white hover:bg-[#009f82]'
                : 'bg-slate-100 text-slate-400 cursor-not-allowed border border-slate-200'
              }`}
          >
            {isSavingAll ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}
      ...
      {isAdmin && hasUnsaved && (
        <div className="flex items-center gap-3 rounded-xl border border-blue-200 bg-blue-50 px-5 py-3">
          ...
          <p className="text-[13px] font-semibold text-blue-700">Có thay đổi chưa lưu — nhấn "Lưu thay đổi" để áp dụng.</p>
        </div>
      )}
      ...
      {loading ? (
        <div className="grid gap-4 lg:grid-cols-5">
          {[1, 2, 3].map(i => <div key={i} className="h-48 rounded-[20px] bg-slate-100 animate-pulse" />)}
        </div>
      ) : (
        <div className="grid gap-4 lg:grid-cols-5">
          ...
          <div className="space-y-4 lg:col-span-3 flex flex-col">
            ...
            <div className={cardClass}>
              ...
              <div className="flex items-center gap-4">
                {user?.avatar ? (
                  <img src={user.avatar} alt="avatar" className="w-14 h-14 rounded-full border-2 border-[#00b494]/30 object-cover shadow-sm" />
                ) : (
                ...
                <div>
                  ...
                  <span className={`inline-block mt-2 text-[11px] font-bold uppercase tracking-wide px-2.5 py-1 rounded-lg ${isAdmin ? 'bg-amber-50 text-amber-600 border border-amber-200' : 'bg-[#00b494]/10 text-[#00b494] border border-[#00b494]/20'
                    }`}>
            ...
            <div className={cardClass}>
              ...
              <div className="space-y-3">
                <label className="block text-[13px] font-bold text-slate-700">Email người thân</label>
                <div className="relative">
                  <Mail className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
                  <input
                    type="email"
                    value={sosEmailInput}
                    onChange={e => { setSosEmailInput(e.target.value); setEmailSaveStatus('idle'); }}
                ...
                <p className="text-[12px] text-slate-400">Gửi email kèm bản đồ GPS khi xảy ra sự cố khẩn cấp.</p>

                {/* Save email button — mọi user đều dùng được */}
                <div className="flex items-center gap-3 pt-1">
                  <button
                    onClick={handleSaveEmail}
                    disabled={isSavingEmail || !sosEmailInput.trim() || sosEmailInput === settings.sos_email}
                    className={`flex items-center gap-2 rounded-xl px-4 py-2 text-[13px] font-bold transition-all active:scale-95 shadow-sm ${isSavingEmail || !sosEmailInput.trim() || sosEmailInput === settings.sos_email
                  ...
                  {emailSaveStatus === 'error' && (
                    <span className="flex items-center gap-1.5 text-[12px] font-semibold text-red-500 animate-in fade-in duration-200">
                      <AlertTriangle className="h-4 w-4" /> Email không hợp lệ hoặc lỗi kết nối.
                    </span>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-7">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <label className="text-[14px] font-bold text-slate-700">Ngưỡng gia tốc va chạm</label>
                ...
                <div className="space-y-3">
                  ...
                  <input
                    ...
                    onChange={e => updateSetting('tilt_threshold', Number(e.target.value))}
                    disabled={!isAdmin}
                ...
                <div className="space-y-3">
                  ...
                  <input
                    ...
                    onChange={e => updateSetting('speed_threshold', Number(e.target.value))}
                    disabled={!isAdmin}
                ...
                <div className="bg-slate-50 rounded-xl p-4 flex gap-3 text-[13px] text-slate-600 border border-slate-100">
                  ...
                  <p>
                    Cảnh báo khi gia tốc vượt <strong>{SENSITIVITY_MAP[settings.sensitivity ?? 3]}G</strong>,
                    nghiêng quá <strong>{settings.tilt_threshold ?? 45}°</strong>,
                    hoặc tốc độ trên <strong>{settings.speed_threshold ?? 80} km/h</strong>.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-4 lg:col-span-2 flex flex-col">
            <div className={`${cardClass} flex-1`}>
              ...
              <div className="space-y-3">
                {[
                  { label: 'Va chạm', value: `> ${SENSITIVITY_MAP[settings.sensitivity ?? 3]}G`, cls: 'text-[#00b494] bg-[#00b494]/10' },
                  { label: 'Góc ngã', value: `> ${settings.tilt_threshold ?? 45}°`, cls: 'text-purple-600 bg-purple-50' },
                  { label: 'Tốc độ cao', value: `> ${settings.speed_threshold ?? 80} km/h`, cls: 'text-amber-600 bg-amber-50' },
                ].map(s => (
                  <div key={s.label} className="flex justify-between items-center bg-white p-3 rounded-xl border border-slate-100 shadow-sm">
                    <span className="text-[13px] text-slate-600 font-medium">{s.label}</span>
                    <span className={`font-bold text-[12px] ${s.cls} px-2 py-0.5 rounded-lg`}>{s.value}</span>
                  </div>
                ))}
              </div>
              ...
              {(settings.sos_email || sosEmailInput) && (
                <div className="mt-5 space-y-2">
                  <p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">SOS đã cấu hình</p>
