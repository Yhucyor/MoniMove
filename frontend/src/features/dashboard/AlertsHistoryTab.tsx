...
import {
  AlertTriangle, Clock, RefreshCw, ShieldCheck, Zap,
  Activity, MapPin, Navigation, Route, Timer, CalendarDays, Search,
} from 'lucide-react';
import { useMyDevices } from '../../hooks/useMyDevices';

const HistoryMapModal = dynamic(() => import('./HistoryMapModal'), { ssr: false });
...
export default function AlertsHistoryTab({ showAllDevices = false }: AlertsHistoryTabProps) {
  ...
  useEffect(() => { fetchLogs(); }, [showAllDevices]);
  ...
  const fetchTodayHistory = async (showRefreshIndicator = false) => {
    if (!deviceId) { setHistoryPoints([]); setTotalDistance(0); return; }
    ...
    try {
      ...
      if (!data) { setHistoryPoints([]); setTotalDistance(0); return; }
      ...
      let dist = 0;
      for (let i = 1; i < allLogs.length; i++) dist += haversineM(allLogs[i - 1], allLogs[i]);
  ...
  useEffect(() => { fetchTodayHistory(); }, [deviceId]);
  ...
  const durationSec = historyPoints.length > 1
    ? Math.round((historyPoints[historyPoints.length - 1].timestamp - historyPoints[0].timestamp) / 1000)
    : 0;
    ...
    <div className="space-y-6 animate-in fade-in duration-300 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-100 pb-5">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400 font-medium">
            {showAllDevices ? 'Toàn bộ sự cố từ mọi thiết bị.' : 'Sự cố và lịch sử di chuyển trong ngày.'}
          </p>
        </div>

        <button
          onClick={() => activeTab === 'alerts' ? fetchLogs(true) : fetchTodayHistory(true)}
        ...
        >
          <RefreshCw className={`h-3.5 w-3.5 text-slate-500 ${isRefreshing || isRefreshingHistory ? 'animate-spin' : ''}`} />
          Làm mới
        </button>
      </div>
      ...
      <div className="flex gap-1 p-1 bg-slate-100 rounded-2xl w-fit">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-all duration-200 ${activeTab === 'alerts'
        ...
        >
          ...
          Sự cố
          {logs.length > 0 && (
            <span className="bg-red-100 text-red-600 text-[9px] font-black px-1.5 py-0.5 rounded-full">{logs.length}</span>
          )}
        </button>
        <button
          onClick={() => setActiveTab('history')}
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-all duration-200 ${activeTab === 'history'
        ...
        >
          ...
          {historyPoints.length > 0 && (
            <span className="bg-[#00b494]/10 text-[#00b494] text-[9px] font-black px-1.5 py-0.5 rounded-full">{historyPoints.length}</span>
          )}
        </button>
      </div>
        ...
        <>
          ...
          {loading ? (
            ...
            <div className="rounded-2xl border border-red-100 bg-red-50/50 p-6 text-center">
              ...
              <button onClick={() => fetchLogs()} className="mt-3 text-xs font-bold text-red-700 underline uppercase cursor-pointer">Thử lại</button>
            </div>
          ) : logs.length === 0 ? (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-start gap-3">
                        <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border ${isCritical ? 'bg-red-50 border-red-100 text-red-500' : 'bg-amber-50 border-amber-100 text-amber-500'}`}>
                          <AlertTriangle className="h-4 w-4" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-wrap items-center gap-2 mb-1.5">
                            <span className={`inline-flex items-center rounded-lg px-2.5 py-0.5 text-[10px] font-extrabold uppercase tracking-wide border ${isCritical ? 'bg-red-50 text-red-600 border-red-100' : 'bg-amber-50 text-amber-600 border-amber-100'}`}>
                              {isCritical ? '🔴' : '🟡'} {alertType || '—'}
                            </span>
                            <span className="text-[10px] font-bold text-slate-400 bg-slate-50 border border-slate-100 px-2 py-0.5 rounded-lg">{log.deviceId}</span>
                          </div>
        ...
        <>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="flex flex-wrap items-end gap-3">
              <div className="flex flex-col gap-1">
                <label className="text-[11px] font-semibold text-slate-500">Ngày</label>
                <input
                  type="date"
                  value={selectedDate}
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[11px] font-semibold text-slate-500">Từ</label>
                <input
                  type="time"
                  value={startTime}
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[11px] font-semibold text-slate-500">Đến</label>
                <input
                  type="time"
                  value={endTime}
          ...
          {!historyLoading && historyPoints.length > 0 && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="rounded-2xl border border-[#00b494]/20 bg-[#00b494]/5 p-4 shadow-sm flex items-center gap-3">
                ...
                <div>
                  <p className="text-[9px] font-bold uppercase tracking-wider text-[#00b494]/70">Điểm GPS</p>
                  <p className="text-xl font-black text-[#00b494]">{historyPoints.length}</p>
                </div>
              </div>
              <div className="rounded-2xl border border-blue-100 bg-blue-50/40 p-4 shadow-sm flex items-center gap-3">
                ...
                <div>
                  <p className="text-[9px] font-bold uppercase tracking-wider text-blue-400">Quãng đường</p>
              ...
              <div className="rounded-2xl border border-purple-100 bg-purple-50/40 p-4 shadow-sm flex items-center gap-3">
                ...
                <div>
                  <p className="text-[9px] font-bold uppercase tracking-wider text-purple-400">Thời gian</p>
                  <p className="text-xl font-black text-purple-600">{durationSec > 0 ? formatDuration(durationSec) : '—'}</p>
                </div>
              </div>
              <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm flex items-center gap-3">
                <div className="h-9 w-9 rounded-xl bg-slate-100 flex items-center justify-center shrink-0">
                ...
                <div>
                  <p className="text-[9px] font-bold uppercase tracking-wider text-slate-400">Bắt đầu</p>
                  <p className="text-sm font-black text-slate-700">
                    {historyPoints.length > 0 ? new Date(historyPoints[0].timestamp).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' }) : '—'}
                  </p>
                </div>
              </div>
            </div>
          ...
          {historyLoading ? (
            <div className="space-y-2">
              {[1, 2, 3, 4, 5].map((i) => (
                <div key={i} className="flex gap-3 p-4 rounded-2xl border border-slate-100 bg-white animate-pulse">
            ...
            <div className="rounded-2xl border border-red-100 bg-red-50/50 p-6 text-center">
              ...
              <button onClick={() => fetchTodayHistory()} className="mt-3 text-xs font-bold text-red-700 underline cursor-pointer">Thử lại</button>
            </div>
          ) : historyPoints.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 border border-dashed border-slate-200 rounded-[24px] bg-white/50 text-center">
              ...
              <h3 className="text-base font-bold text-slate-800 mb-1">Chưa có lịch sử di chuyển hôm nay</h3>
              <p className="text-sm text-slate-400 max-w-[300px] leading-relaxed">
              ...
              <div className="space-y-2 pl-14 max-h-[520px] overflow-y-auto pr-1">
                {historyPoints.map((pt, idx) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`absolute -left-[38px] top-4 h-3.5 w-3.5 rounded-full border-2 border-white shadow-sm ${isFirst || isLast ? 'bg-[#00b494] scale-125' : 'bg-slate-300 group-hover:bg-[#00b494]/60'} transition-all`} />

                      <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg ${isFirst ? 'bg-[#00b494] text-white' : isLast ? 'bg-blue-500 text-white' : 'bg-slate-100 text-slate-400 group-hover:bg-[#00b494]/10 group-hover:text-[#00b494]'} transition-colors`}>
                            <MapPin className="h-3.5 w-3.5" />
                          </div>
                          <div className="min-w-0">
                            <p className="text-[11px] font-bold text-slate-700 truncate">
                              {isFirst ? '🟢 Xuất phát' : isLast ? '🔵 Điểm cuối' : `Điểm ${idx + 1}`}
                            </p>
