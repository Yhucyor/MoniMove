...
import { RefreshCw, Radio, Cpu, LayoutGrid, List, Search, Filter } from 'lucide-react';
    ...
    <div className="space-y-6 animate-in fade-in duration-300 max-w-6xl mx-auto">
      ...
      {!loading && !listLoading && devices.length > 0 && (
        <div className="grid grid-cols-3 gap-3">
          <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm flex items-center gap-3">
            ...
            <div>
              <p className="text-[9px] font-bold uppercase tracking-wider text-slate-400">Tổng thiết bị</p>
          ...
          <div className="rounded-2xl border border-emerald-100 bg-emerald-50/40 p-4 shadow-sm flex items-center gap-3">
            ...
            <div>
              <p className="text-[9px] font-bold uppercase tracking-wider text-emerald-500">Trực tuyến</p>
              <p className="text-xl font-black text-emerald-600">{onlineCount}</p>
            </div>
          </div>
          <div className="rounded-2xl border border-slate-100 bg-slate-50/50 p-4 shadow-sm flex items-center gap-3">
            ...
            <div>
              <p className="text-[9px] font-bold uppercase tracking-wider text-slate-400">Offline</p>
              <p className="text-xl font-black text-slate-500">{devices.length - onlineCount}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {(loading || listLoading) ? (
        <div className={viewMode === 'grid' ? "grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3" : "flex flex-col gap-4"}>
          {[1, 2, 3].map((skeleton) => (
            <div key={skeleton} className="w-full rounded-[24px] border border-slate-100 bg-white p-6 shadow-sm animate-pulse space-y-4">
        ...
        <div className="rounded-2xl border border-red-100 bg-red-50/50 p-6 text-center max-w-md mx-auto">
          ...
          <p className="text-sm font-semibold text-red-600">{error || listError}</p>
          <button
        ...
        <div className="flex flex-col items-center justify-center py-16 border border-dashed border-slate-200 rounded-[24px] bg-white/50 text-center">
          ...
          <p className="text-sm font-bold text-slate-600">Không tìm thấy thiết bị nào</p>
