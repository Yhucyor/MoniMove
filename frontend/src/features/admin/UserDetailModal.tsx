...
function GpsHistoryMap({
  points,
  deviceId,
}: {
  points: DevicePosition[];
  deviceId: string;
}) {
  ...
  useEffect(() => {
    if (!mapRef.current || typeof window === "undefined" || points.length === 0)
      return;
    ...
    Promise.all([import("leaflet")]).then(([L]) => {
      ...
      if (points.length > 0) {
        ...
        L.marker(
          ...
          .addTo(map)
          .bindPopup("Điểm cuối / Vị trí mới nhất")
          .openPopup();
      }
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-4 py-2.5 text-xs font-bold rounded-t-xl transition-all border-b-2 ${
                  activeTab === tab.id
              ...
              <div className="rounded-2xl border border-slate-100 bg-slate-50/50 divide-y divide-slate-100 overflow-hidden">
                {[
                  ...
                  },
                  {
                    icon: Cpu,
                    label: "Số thiết bị",
                    value: `${user.deviceIds.length} thiết bị được cấp phép`,
                  },
                ].map((row) => (
                  <div
                    key={row.label}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {row.badge ? (
                        <span
                          className={`mt-0.5 inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-bold border ${
                            row.badge === "amber"
                ...
                <div className="rounded-2xl border border-slate-100 bg-white p-5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {user.deviceIds.map((id) => (
                      <span
                        key={id}
                        className="inline-flex items-center gap-1.5 rounded-xl bg-purple-50 border border-purple-100 px-3 py-1.5 text-xs font-bold text-purple-700"
                      >
              ...
              {user.deviceIds.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 border border-dashed border-slate-200 rounded-2xl bg-slate-50">
                  ...
                  <p className="text-sm font-bold text-slate-500">
                    Người dùng này chưa có thiết bị
                  </p>
                  <p className="text-xs text-slate-400 mt-1">
                    Gán thiết bị ở tab Quản lý tài khoản
                  </p>
                </div>
              ) : (
                <>
                    ...
                    <div className="rounded-2xl border border-slate-100 bg-white overflow-hidden">
                      <div className="px-4 py-3 border-b border-slate-100 flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-slate-700">
                          Chi tiết hành trình ({historyPoints.length} điểm)
                        </span>
                      </div>
                          ...
                          .map((pt, i) => (
                            ...
                            >
                              ...
                              <div className="text-right shrink-0">
                                {pt.speed !== undefined && (
                                  <span
                                    className={`text-[10px] font-bold ${pt.speed > 60 ? "text-red-500" : "text-emerald-600"}`}
                                  >
                ...
                <>
                  ...
                  <div className="space-y-2">
                    {pagedAlerts.map((alert) => (
                      <div
                        key={alert.id}
                        className="rounded-xl border border-slate-100 bg-white p-3.5 flex items-start gap-3 hover:border-amber-200 transition-colors"
                      >
                  ...
                  {totalAlertPages > 1 && (
                    <div className="flex items-center justify-between pt-1">
                      <p className="text-xs text-slate-400">
                        Trang {currentPage} / {totalAlertPages}
                      </p>
