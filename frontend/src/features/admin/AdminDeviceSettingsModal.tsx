...
import {
    X, Settings, Save, RefreshCw, Mail, Phone, Sliders, Compass,
    Zap, Volume2, AlertTriangle, CheckCircle, Info,
} from 'lucide-react';
import { DeviceListItem, DeviceSettings, getDeviceSettings, saveDeviceSettings } from '../../services/api';
...
export default function AdminDeviceSettingsModal({ device, onClose }: AdminDeviceSettingsModalProps) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            setHasUnsaved(false);
            setToast({ type: 'success', message: `Đã lưu cài đặt cho ${device.name}` });
    ...
    const isOnline = device.status === 'online' || device.status === 'active' || device.connectionStatus === 'online';
            ...
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-xl flex flex-col max-h-[92vh] overflow-hidden">
                ...
                <div className="flex items-center justify-between px-6 pt-6 pb-5 border-b border-slate-100 shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-base font-extrabold text-slate-900">Cài đặt thiết bị</h2>
                            <div className="flex items-center gap-2 mt-0.5">
                                <span className="text-xs text-slate-500 font-mono">{device.name}</span>
                                <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[9px] font-bold border ${isOnline ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-slate-100 text-slate-500 border-slate-200'
                                    }`}>
                ...
                <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
                    ...
                    {loading ? (
                        <div className="space-y-4">
                            {[1, 2, 3, 4].map(i => <div key={i} className="h-14 rounded-2xl bg-slate-100 animate-pulse" />)}
                        </div>
                    ) : (
                        <>
                            {/* Unsaved warning */}
                            {hasUnsaved && (
                                <div className="flex items-center gap-2.5 rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5">
                                    ...
                                    <p className="text-xs font-semibold text-blue-700">Có thay đổi chưa lưu</p>
                                </div>
                            )}
                            ...
                            <section className="rounded-2xl border border-slate-100 bg-white p-5 space-y-4">
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    />
                                    <p className="text-[11px] text-slate-400">Nhận email khẩn cấp kèm vị trí GPS khi xảy ra sự cố nghiêm trọng.</p>
                                </div>
                            ...
                            <section className="rounded-2xl border border-slate-100 bg-white p-5 space-y-5">
                                ...
                                <div className="space-y-3">
                                    <div className="flex items-center justify-between">
                                        <label className="text-sm font-bold text-slate-700">Độ nhạy va chạm</label>
                                        <span className="rounded-lg bg-[#00b494]/10 px-2.5 py-1 text-xs font-bold text-[#00b494]">
                                            Cấp {settings.sensitivity} — {SENSITIVITY_MAP[settings.sensitivity ?? 3]}G
                                        </span>
                                    </div>
                                    <input
                            ...
                            <section className="rounded-2xl border border-slate-100 bg-white p-5 space-y-4">
                                ...
                                {[
                                    ...
                                    <div key={item.key as string} className="flex items-center justify-between gap-4">
                                        ...
                                        <label className="relative inline-flex items-center cursor-pointer shrink-0">
                                            ...
                                            />
                                            <div className={`peer h-6 w-11 rounded-full bg-slate-200 after:absolute after:left-[2px] after:top-[2px] after:h-5 after:w-5 after:rounded-full after:bg-white after:transition-all after:content-[''] peer-checked:after:translate-x-5 peer-focus:outline-none peer-disabled:opacity-50 ${item.color === 'purple' ? 'peer-checked:bg-purple-500' : 'peer-checked:bg-blue-500'
                                                }`} />
                                        </label>
                                    </div>
                                ))}
                            ...
                            <section className="rounded-2xl border border-slate-100 bg-slate-50/50 p-4">
                                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-3">Ngưỡng hiện tại</p>
                                <div className="grid grid-cols-3 gap-2">
                                    {[
                                        { label: 'Va chạm', value: `>${settings.accel_threshold ?? 2.5}G`, color: 'text-[#00b494] bg-[#00b494]/10' },
                                        { label: 'Góc ngã', value: `>${settings.tilt_threshold ?? 45}°`, color: 'text-purple-600 bg-purple-50' },
                                        { label: 'Tốc độ', value: `>${settings.speed_threshold ?? 80}km/h`, color: 'text-amber-600 bg-amber-50' },
                                    ].map(s => (
                                        <div key={s.label} className="rounded-xl border border-slate-100 bg-white p-2.5 text-center">
                                            <p className="text-[10px] font-bold text-slate-400">{s.label}</p>
                                            <span className={`mt-1 inline-block text-xs font-black rounded-lg px-2 py-0.5 ${s.color}`}>{s.value}</span>
                                        </div>
                                    ))}
                                </div>
                            </section>
                ...
                <div className="flex items-center justify-between gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50/50 shrink-0">
                    {/* Toast inline */}
                    {toast ? (
                        <div className={`flex items-center gap-2 text-xs font-bold ${toast.type === 'success' ? 'text-emerald-700' : 'text-red-700'}`}>
                            {toast.type === 'success'
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            ...
                            className={`flex items-center gap-2 px-5 py-2 rounded-xl text-xs font-bold shadow-sm transition-all active:scale-95 ${hasUnsaved && !saving
