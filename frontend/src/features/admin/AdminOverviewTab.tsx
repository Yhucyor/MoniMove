...
import { Users, Cpu, AlertTriangle, Activity, TrendingUp, Shield, RefreshCw, Clock, Radio, Zap } from 'lucide-react';
import { getAllUsers, listDevices, getAlertsHistory, AlertLog, DeviceListItem } from '../../services/api';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts';
import { useWebSocket } from '../../hooks/useWebSocket';

export default function AdminOverviewTab() {
  const [stats, setStats] = useState({ users: 0, devices: 0, alerts: 0, online: 0 });
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      setStats({
        ...
        online: deviceList.filter((d) => d.connectionStatus === 'online' || d.status === 'online' || d.status === 'active').length,
      });
  ...
  const alertChartData = Object.entries(alertTypeMap).map(([name, count]) => ({ name, count }));

  // Device status chart
  const deviceChartData = [
    { name: 'Trực tuyến', value: stats.online, fill: '#00b494' },
    { name: 'Ngoại tuyến', value: stats.devices - stats.online, fill: '#ef4444' },
  ];
  ...
  const statCards = [
    ...
    },
    {
      ...
      trend: stats.devices > 0 ? `${Math.round((stats.online / stats.devices) * 100)}%` : '0%',
    },
    {
      label: 'Tổng sự cố',
      value: stats.alerts,
      ...
      bg: stats.alerts > 0 ? 'bg-amber-50 border-amber-100' : 'bg-emerald-50 border-emerald-100',
      trend: 'hôm nay',
    },
  ];
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-slate-100 pb-5">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-400 font-medium">
            Theo dõi toàn bộ dữ liệu người dùng, thiết bị và sự cố trong hệ thống MoniMove.
          </p>
        </div>
          ...
          return (
            <div key={card.label} className={`rounded-2xl border ${card.bg} p-5 shadow-sm`}>
              <div className="flex items-center justify-between mb-3">
                ...
                <span className={`text-[10px] font-bold ${card.color} opacity-70`}>{card.trend}</span>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
          ...
          {stats.devices > 0 ? (
              ...
              <div className="flex items-center gap-3 mb-4">
                <div className="flex-1 h-3 rounded-full bg-slate-100 overflow-hidden">
                  <div
                    ...
                    style={{ width: `${(stats.online / stats.devices) * 100}%` }}
                  />
                </div>
            ...
            <p className="text-xs text-slate-400 text-center py-8">Chưa có dữ liệu thiết bị</p>
          )}
        </div>
        ...
        <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
          ...
          {alertChartData.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-8">
              ...
              <p className="text-xs text-slate-400 font-semibold">Không có sự cố nào gần đây</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
          ...
          {loading ? (
            ...
            <p className="text-xs text-slate-400 text-center py-6">Chưa có sự cố nào</p>
          ) : (
            <div className="space-y-2">
              {recentAlerts.map((alert) => (
                <div key={alert.id} className="flex items-start gap-3 rounded-xl border border-slate-100 bg-slate-50/50 p-3">
                  <AlertTriangle className="h-4 w-4 text-amber-500 mt-0.5 shrink-0" />
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-[10px] font-bold text-amber-600 truncate">{alert.alertType || 'Sự cố'}</span>
                      <span className="text-[9px] text-slate-400 shrink-0">{alert.deviceId}</span>
                    </div>
                    <p className="text-[10px] text-slate-600 mt-0.5 truncate">{alert.message}</p>
        ...
        <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
          ...
          {loading ? (
            ...
            <p className="text-xs text-slate-400 text-center py-6">Chưa có thiết bị</p>
          ) : (
            <div className="space-y-2">
              {devices.map((d) => {
                ...
                return (
                  <div key={d.id} className="flex items-center justify-between rounded-xl border border-slate-100 bg-slate-50/50 p-3">
                    ...
                    <span className={`text-[9px] font-bold uppercase rounded-full px-2 py-0.5 ${isOnline ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-500'
                      }`}>
      ...
      <div className="rounded-2xl border border-slate-100 bg-gradient-to-r from-slate-50 to-white p-4 shadow-sm">
        <div className="flex flex-wrap items-center gap-4 text-[10px] font-semibold text-slate-500 uppercase tracking-wider">
          ...
          <div className="flex items-center gap-1.5">
            <Zap className={`h-3 w-3 ${isConnected ? 'text-amber-500' : 'text-slate-400'}`} />
            WebSocket: {isConnected ? `Live` : connectionState}
            {wsAlertCount > 0 && <span className="text-amber-600 ml-1">+{wsAlertCount} alerts</span>}
          </div>
          <div className="ml-auto text-[9px] text-slate-400">
            MoniMove v1.0 · {new Date().getFullYear()}
          </div>
        </div>
      </div>
    </div>
