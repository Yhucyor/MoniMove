...
import { MapPin, Navigation, Layers, Gauge, Shield, Clock, Route, Locate, X, Plus, Minus, RefreshCw, Compass, Crosshair, ExternalLink } from 'lucide-react';
...
export default function MapComponent({ showRoute = true, showSafeZone = true }: MapComponentProps) {
  ...
  useEffect(() => {
    // Start periodic position updates after ensuring permission
...
const userIcon = useMemo(() => {
  ...
  return (
    <div className={`absolute inset-0 ${isDanger ? 'border-4 border-red-500' : ''}`}>
      ...
      <div className="absolute bottom-6 right-4 z-[1000] flex flex-col gap-1.5">
        ...
        <div className="flex flex-col bg-white/95 backdrop-blur-md rounded-lg shadow-md border border-slate-200/50 overflow-hidden">
          ...
          <button
            ...
            className={`w-8 h-8 flex items-center justify-center transition-all active:scale-95 cursor-pointer ${showStraightLine ? 'text-red-500 bg-red-50' : 'text-slate-600 hover:text-red-600 hover:bg-slate-50'}`}
            title="Đường thẳng"
          >
            <Minus className="w-3.5 h-3.5" />
          </button>
